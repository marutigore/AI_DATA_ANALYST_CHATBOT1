...
class ChatRequest(BaseModel):
    session_id: str = Field(..., description="Unique session UUID returned upon dataset upload", json_schema_extra={"example": "123e4567-e89b-12d3-a456-426614174000"})
    prompt: str = Field(..., min_length=1, max_length=2000, description="Natural language analytical query", json_schema_extra={"example": "Show total sales by region"})


class ChatResponse(BaseModel):
    response: str = Field(..., description="Conversational or analytical text response generated by LLM agent")
    plot_json: Optional[str] = Field(None, description="Serialized Plotly figure JSON string if a chart was generated")
    citations: Optional[list] = Field(None, description="Retrieved tabular context chunks used as citations")

class DatasetMetrics(BaseModel):
    rows: int = Field(..., description="Total row count of uploaded dataset")
    cols: int = Field(..., description="Total column count of uploaded dataset")
    memory_mb: float = Field(..., description="Memory footprint of DataFrame in megabytes")
    missing: int = Field(..., description="Total count of missing or null cells")

class UploadResponse(BaseModel):
    session_id: str = Field(..., description="Generated session UUID for subsequent chat queries")
    filename: str = Field(..., description="Original filename of uploaded CSV/Excel document")
    metrics: DatasetMetrics = Field(..., description="Calculated profiling metrics")
    preview_json: str = Field(..., description="JSON serialized preview of first 40 rows")
    suggestions: list = Field(..., description="Heuristic query suggestions")

class ErrorDetail(BaseModel):
    detail: str = Field(..., description="Human readable error message detailing validation or execution failure")
